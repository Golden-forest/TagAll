...
const uiText = {
  navLabel: { zh: '章节导航', en: 'Chapter navigation' },
} as const

export function PhotoAlbumChapterNav({ chapters, visible }: Props) {
  const { lang } = useLang()
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (intersecting[0]) {
          const id = intersecting[0].target.id.replace('chapter-', '')
  ...
  return (
    <nav
      ...
      aria-label={t(uiText.navLabel, lang)}
    >
