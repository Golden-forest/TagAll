...
function assertHref(value, path, { allowMailto = false } = {}) {
  ...
  if (value.startsWith('#') || (allowMailto && value.startsWith('mailto:'))) {
    return
  }

  const expected = allowMailto ? 'an anchor or mailto link' : 'an anchor link'
...
if (errors.length > 0) {
  console.error(`Invalid Mirra content:\n${errors.map((error) => `- ${error}`).join('\n')}`)
  process.exit(1)
}
